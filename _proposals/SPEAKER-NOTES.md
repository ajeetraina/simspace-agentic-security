...
## Slide 35 - Try it - SBOM, VEX and SLSA: your turn to generate an SBOM, apply VEX, and ve...
`slide-try-lab2.webp`

Your turn - this is the hands-on for **SBOM, VEX and SLSA**. In the workshop, generate an SBOM for the catalog image, apply a **VEX** statement to strip the CVE noise down to what's actually exploitable, and verify the **SLSA** provenance - the three building blocks we just walked through, now on your own terminal. Run them against the baseline and the hardened image and watch the difference. Once you've done it, come back and we'll mark the BUILD stage green.

## Slide 36 - The journey, checkpoint 1 of 4: Lab 1 done, the BUILD stage is now green - yo...
...
## Slide 37 - Question 2 of 4 - Baseline: did it start from something trustworthy? Docker H...
...
## Slide 42 - Catalog service, where vulnerabilities enter: without the DHI MCP the agent p...
`slide-11.webp`
...
## Slide 45 - Question 3 of 4 - Gate: is it allowed to pass? Build policies, image signing,...
...
## Slide 53 - Try it - Verify it. Gate it.: your turn to write a Docker Scout policy and ma...
`slide-try-lab3.webp`

Your turn - the **Verify it, Gate it** hands-on. In the workshop, attach and verify the attestations, then write the `docker-scout-policy.yaml` and run it against both images: watch the ungoverned `:baseline` fail the gate and the hardened `:dhi` pass. Then wire that same policy into the CI workflow so the pipeline fails closed on every push - no human eyeballing required. Once the gate holds, come back and we'll mark SIGN, GATE and DEPLOY green.

## Slide 54 - The journey, checkpoint 3 of 4: Lab 3 done, SIGN, GATE and DEPLOY are now gre...
...
## Slide 55 - Question 4 of 4 - Boundary: what could it reach while it worked? Sandbox runt...
...
## Slide 57 - Agent with a Sandbox: sbx microVM boundary, DHI MCP server, hardened base, ze...
`slide-10.webp`
...
## Slide 58 - Sandboxes (experimental): run agents in isolation rather than on your bare ma...
...
## Slide 59 - Sandbox architecture: on the host machine, the agent container runs inside a ...
...
## Slide 60 - The Sandbox TUI: Docker Sandboxes live view - sandboxes on the left (claude-d...
...
## Slide 61 - Managing credentials: always prefer stored secrets over env vars (keychain en...
...
## Slide 62 - Protecting MCP: tools are how agents act on the world - govern which servers ...
...
## Slide 63 - The agent talks to one gateway, never to servers directly: the sandboxed agen...
...
## Slide 64 - Point it at a real gateway: local gateway http://localhost:8811 (Compose or D...
...
## Slide 65 - Author once in Docker Hub, enforce around the sandbox and at the gateway: Doc...
...
## Slide 66 - The server lifecycle - five commands: sbx mcp add to register, sbx mcp ls/ins...
...
## Slide 67 - Default-deny allow-list over (server, tool), authored in Cedar: a permit poli...
